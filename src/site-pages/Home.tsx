      ...
      <main>
        <section className="relative min-h-[760px] overflow-hidden bg-[#202020] pt-28 text-white lg:min-h-[820px] lg:pt-32">
          ...
          <div className="container relative flex min-h-[620px] flex-col justify-center py-16 lg:min-h-[680px]">
            <p className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[.2em] text-[#a8d8ea]"><span className="h-px w-8 bg-current" />Groupe LVMR</p>
            <h1 className="mt-6 max-w-[850px] text-[clamp(3rem,8vw,6.6rem)] font-extrabold leading-[.9] tracking-[-.065em]">L’excellence en<br/><span className="text-[#ebd899]">toutes circonstances.</span></h1>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link href="/devis" className="inline-flex min-h-13 items-center gap-2 rounded-xl bg-[#dab844] px-7 text-[14px] font-extrabold text-[#202020] transition hover:bg-[#e2c86f]">Demander un devis <ArrowRight size={16}/></Link>
          ...
          <div className="absolute inset-x-0 bottom-0 border-t border-white/12 bg-[#202020]/75 backdrop-blur-xl">
            <div className="container flex flex-wrap items-center justify-between gap-4 py-5 text-[12px] font-semibold text-white/65">
              <span className="flex items-center gap-2"><MapPin size={15} className="text-[#a8d8ea]"/> Saint-Germain-en-Laye · Paris · Île-de-France</span>
        ...
        <section className="py-16 sm:py-24">
          <div className="container">
            ...
            <div className="mt-12 grid gap-5 lg:grid-cols-2">
              {[
                { title: "LVMR Premium", text: "Propreté professionnelle haut de gamme : entretien régulier, copropriétés, remises en état et surfaces vitrées.", image: imageSet.premium, href: "/premium", label: "Découvrir LVMR Premium", tone: "#dab844" },
                { title: "LVMR Environnement", text: "Interventions techniques, situations sensibles, nettoyage industriel, hottes professionnelles et solutions 3D.", image: imageSet.environnement, href: "/environnement", label: "Découvrir LVMR Environnement", tone: "#9dd573" },
              ].map((item) => (
        ...
        <section className="bg-white py-16 sm:py-24">
          <div className="container">
            ...
            <div className="mt-10 grid gap-px overflow-hidden rounded-[26px] border border-[#202020]/8 bg-[#202020]/8 md:grid-cols-2 lg:grid-cols-4">
              {commitments.map(([Icon,title,text],index)=><article key={title} className="bg-white p-7"><span className="text-[11px] font-bold text-[#4caac9]">0{index+1}</span><Icon className="mt-8 text-[#4caac9]" size={24}/><h3 className="mt-5 text-[1.25rem] font-extrabold">{title}</h3><p className="mt-3 text-[13px] leading-6 text-[#424242]">{text}</p></article>)}
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#202020] py-16 text-white sm:py-20">
          <div className="container grid gap-9 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
            <div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#a8d8ea]">Parlons de votre projet</p><h2 className="mt-5 max-w-[760px] text-[clamp(2.3rem,5vw,4.8rem)] font-extrabold leading-[.95] tracking-[-.055em]">Entretien, remise en état ou intervention spécialisée ?</h2></div>
            <div><p className="text-[15px] leading-7 text-white/60">Notre équipe étudie votre demande et vous oriente vers le pôle adapté.</p><Link href="/devis" className="mt-7 inline-flex min-h-13 items-center gap-2 rounded-xl bg-[#dab844] px-7 text-[14px] font-extrabold text-[#202020]">Parler de votre projet <ArrowRight size={16}/></Link></div>
          </div>
        </section>
      </main>
      <SiteFooter />
      <Link href="/devis" className="fixed bottom-4 left-4 right-4 z-40 flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#dab844] text-[14px] font-extrabold text-[#202020] shadow-xl lg:hidden">Demander un devis <ArrowRight size={15}/></Link>
    </div>
  );
}
